...
{	
	...
	// or requal to the given period. The duty cycle cannot exceed 99%.
	if (duty_cycle >= period_constant) return;
	
	// Enable the clock to PWM Module 1 by setting the 
	// R1 bit (bit 1) in the FRCGCPWM register
	SYSCTL->RCGCPWM |= 0x02;
	...
	// Configure the PF2 pin to operate as a Module 1 PWM6 (M1PWM6)
	...
	// use the Count-Down mode by clearing the MODE bit (Bit 1)
	...
	// the comparator (i.e the value in PWN3CMPA) while counting down
	PWM1->_3_GENA |= 0xC0;
  ...
  // to drive the PWM signal low when the counter matches the value
	...
	// in the PWM3CMPA register. when the counter matches the vlaue in this register,
